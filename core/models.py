...
class Dmodule(nn.Module):
    def __init__(self):
        super().__init__()
        self.conv = nn.Conv2d(15, 14, kernel_size=1)
        self.norm = torch.nn.GroupNorm(1,14)

        self.deform = nn.Sequential(nn.ReLU(),self.conv,self.norm)

    def forward(self, input, time_emb):
        h = torch.cat([input, time_emb], 1).float()
        res = self.deform(h)
        return input + res
...
class LGM(nn.Module):
    ...
    def forward_gaussians(self, images):
        # images: [B, 4, 9, H, W]      [1, 4, 9, H, W]
        # return: Gaussians: [B, dim_t]

        B, V, C, H, W = images.shape
        images = images.view(B*V, C, H, W)      # [4, 9, H, W]
        ...
        x = x.permute(0, 1, 3, 4, 2).reshape(B, -1, 14)     # [1, 4*14*ssize*ssize, 14]
    ...
    def forward_4dgaussians(self, Dm, images, time):
        # images: [B, 4, 9, H, W]      [1, 4, 9, H, W]
        # return: Gaussians: [B, dim_t]
        ...
        time = torch.tensor(time).repeat(x.shape[0], 1, x.shape[2], x.shape[3]).cuda()
        time_emb = (time.float()  - 4) / 10 # hack
        x =Dm(x, time_emb)    # 自己加的，最x进行一下调整！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！
