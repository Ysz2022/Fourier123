...
class Zero123(nn.Module):
    ...
    def refine(self, pred_rgb, elevation, azimuth, radius, 
        ...
        if strength == 0:
            ...
            t = self.scheduler.timesteps[init_step]
        ...
        # t_in = torch.cat([t.view(1).to(self.device)] * 2)
        # noise_pred = self.unet(
        #     torch.cat([x_in, vae_emb], dim=1),
        #     t_in.to(self.unet.dtype),
        #     encoder_hidden_states=cc_emb,
    ...
    def train_step(self, pred_rgb, elevation, azimuth, radius, step_ratio=None, guidance_scale=5, as_latent=False, default_elevation=0):
        ...
        # # Calculate fourier
        # fourier_image = torch.fft.fft2(latents, dim=(2, 3))
        # fourier_gt = torch.fft.fft2(target.half(), dim=(2, 3))
        # loss = 0.01 * F.mse_loss(torch.abs(fourier_image), torch.abs(fourier_gt)) / latents.shape[0]


        # imgs = self.decode_latents(target.half()) # [1, 3, 512, 512]
        # # Img to Numpy
        # for i in range(imgs.shape[0]):
        #     img = imgs[i]
        #     image = img.cpu().clone()
        #     image = transforms.ToPILImage()(image).resize((512,512)) # 自动转换为0-255
        #     image.save('visual/123/'+str(step_ratio)+str(i)+'.png')
