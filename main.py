...
class GUI:
    def __init__(self, opt):
        ...
        if self.opt.load is not None:
            ...
            print('initialize gaussians to a blob!')
    ...
    # 创建权重矩阵
    def create_weight_matrix(self, size):
        # 中心点坐标
        ...
        # 反距离加权，距离中心越近权重越大
        weight_matrix = 1 / (distance + 1)  # 避免除以零
    ...
    def prepare_train(self):
        ...
        if self.guidance_sd is None and self.enable_sd:
            ...
            # self.guidance_sd1 = MVDream(self.device)
        ...
        if self.input_img is not None:
            ...
            # self.fourier_gt = torch.fft.fft2(torch.mean(self.input_img_torch, dim=1).unsqueeze(0))
        ...
        with torch.no_grad():
            ...
            if self.enable_sd:
                if self.opt.imagedream:
                    ...
                    # self.guidance_sd1.get_text_embeds([self.prompt], [self.negative_prompt])
    ...
    def train_step(self):
        ...
        for _ in range(self.train_steps):
            ...
            if self.input_img_torch is not None and not self.opt.imagedream:
                ...
                if self.enable_sd:
                    # fourier loss
                    fourier_image = torch.fft.fft2(image, dim=(2, 3))
                    # loss = loss + 10 * F.mse_loss(torch.abs(fourier_image), torch.abs(self.fourier_gt))
                    loss = loss + 30 * F.l1_loss(fourier_image, self.fourier_gt)
            ...
            for _ in range(self.opt.batch_size):
                ...
                if self.opt.mvdream or self.opt.imagedream:
                    print('mvdream is using!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
                    for view_i in range(0, 4):
                        # pose_i = orbit_camera(self.opt.elevation + ver, hor + 90 * view_i, self.opt.radius + radius)
                        pose_i = orbit_camera(self.opt.elevation, 90 * view_i, self.opt.radius + radius)
                        poses.append(pose_i)
            ...
            if self.enable_sd:
                if self.opt.mvdream:
                    loss = loss + self.opt.lambda_sd * self.guidance_mvd.train_step(images[1:], camera=poses[1:],step_ratio=step_ratio if self.opt.anneal_timestep else None)
                else:
                    loss_fsd = self.opt.lambda_sd * self.guidance_sd.train_step(images, step_ratio)
                    loss = loss + loss_fsd


            if self.enable_zero123:  # True
                # print('zero123')
                loss_zero123 = self.opt.lambda_zero123 * self.guidance_zero123.train_step(images, vers, hors, radii, step_ratio=step_ratio if self.opt.anneal_timestep else None, default_elevation=self.opt.elevation)
                loss = loss + loss_zero123

            if self.step % 40 == 0:
                print(f"Train Step: {self.step}, loss_zero123: {loss_zero123}, loss_fsd: {loss_fsd}")
            ...
            # densify and prune
            # if self.opt.load is not None:
            #     self.opt.density_start_iter = self.opt.density_start_iter + 100
    ...
    def load_input(self, file):
        ...
        mask = img[..., -1] > 0
        # recenter
        img = recenter(img, mask, border_ratio=0.2)
        ...
        kiui.write_image(os.path.join(file.split('.')[0] + '_recenter.png'), self.input_img)

        # load prompt
        file_prompt = file.replace(".png", "_caption.txt")
...
if __name__ == "__main__":
    ...
    # opt.load = "../LGM-main/workspace_test/anya_rgba.ply"
