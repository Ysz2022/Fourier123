...
def process(opt: Options, path):
    ...
    kiui.write_image(os.path.join(opt.workspace, name + '.png'), image)
    ...
    with torch.no_grad():
        ...
        if opt.fancy_video:
            ...
            for azi in tqdm.tqdm(azimuth):
                ...
                image = model.gs.render(gaussians, cam_view.unsqueeze(0), cam_view_proj.unsqueeze(0), cam_pos.unsqueeze(0), scale_modifier=1)['image'].squeeze(1)
                images.append((image.permute(0, 2, 3, 1).contiguous().float().cpu().numpy() * 255).astype(np.uint8))
                if azi % 5 ==0:
                    image_list.append(transforms.ToPILImage()(image.squeeze(0)))

        images = np.concatenate(images, axis=0)
        imageio.mimwrite(os.path.join(opt.workspace, name + '.mp4'), images, fps=30)
        export_to_gif(image_list, os.path.join(opt.workspace, name + '.gif'))
